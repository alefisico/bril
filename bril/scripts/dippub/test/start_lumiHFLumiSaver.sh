#!/bin/bash
#
# Example of running the LumiHFLumiSaver.
#
...
# determine absolute path to liblumiHFLumiSaver.so
modpath=`find "${base}/src/lumi/lumiHFLumiSaver" -name liblumiHFLumiSaver.so`
[ -z "$modpath" ] && \
   { echo "FATAL: liblumiHFLumiSaver.so not found (did you forgot to compile it first?)" 1>&2; exit 1; }
...
xmlpath="/tmp/test_liblumiHFLumiSaver_${UID}.xml"
...
<xc:Partition xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
   ...
   <xc:Context url="http://localhost:1903">
      ...
      <xc:Application class="lumi::LumiHFLumiSaver" id="254" instance="0" network="local"
                      bus="hf" service="LumiHFLumiSaver"/>
