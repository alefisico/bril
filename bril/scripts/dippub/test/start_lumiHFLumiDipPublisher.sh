#!/bin/bash
#
# Example of running the LumiHFLumiDipPublisher.
#
...
# determine absolute path to liblumiHFLumiDipPublisher.so
modpath=`find "${base}/src/lumi/lumiHFLumiDipPublisher" -name liblumiHFLumiDipPublisher.so`
[ -z "$modpath" ] && \
   { echo "FATAL: liblumiHFLumiDipPublisher.so not found (did you forgot to compile it first?)" 1>&2; exit 1; }
...
xmlpath="/tmp/test_liblumiHFLumiDipPublsiher_${UID}.xml"
...
<xc:Partition xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
   ...
   <xc:Context url="http://localhost:1903">
      <xc:Endpoint protocol="utcp" service="b2in" hostname="localhost" port="2002"
      ...
      <xc:Application class="lumi::LumiHFLumiDipPublisher" id="255" instance="0" network="local"
                      bus="hf" service="LumiHFLumiDipPublisher"/>
...
# start LumiHFLumiDipPublisher
exec xdaq.exe -h localhost -p 1903 -c $xmlpath
